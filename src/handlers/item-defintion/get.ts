...
export const listDefinitions = async (req: Request, resp: Response) => {
	const showDeleted = Boolean(req.query['show-deleted']) || false;
	const definitions = await ItemDefinitionRepository.getAll({ showDeleted });
	return resp.status(200).json({ definitions });
};

export const getDefinition = async (req: Request, resp: Response) => {
	const { id } = req.params;
	const [ err, definition ] = await to<ItemDefinition>(ItemDefinitionRepository.get(id));
	...
	return resp.status(404).json({ error: `could not find definition with id ${id}` });
};
